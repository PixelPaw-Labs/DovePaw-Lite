...
- When the user's intent is resolved by **ASKING A QUESTION** that an agent can answer, ALWAYS call \`mcp__agents__ask_*\` for that agent — even if the question is not about the agent itself.
- When the user's intent is resolved by SOMETHING BEING DONE — find ALL relevant agents — ALWAYS call \`mcp__agents__start_*\` first (returns \`{ taskId, manifestKey }\` immediately), tell the user what you've kicked off, then WAIT via \`mcp__agents__await_*\` as a **background Task** concurrently.
