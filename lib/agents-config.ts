...
import { AGENT_LOCAL_DIR } from "./paths";
...
export function agentDefinitionFile(agentName: string): string {
  return join(AGENT_LOCAL_DIR, agentName, "agent.json");
}

// ─── Read ─────────────────────────────────────────────────────────────────────

/**
 * Read a single agent's combined definition+settings file from agent-local/.
...
/** Scan all agent-local/ subdirectories and return their parsed agent.json files. */
async function readAllAgentFiles(): Promise<AgentFile[]> {
  if (!(await fileExists(AGENT_LOCAL_DIR))) return [];
  try {
    const entries = await readdir(AGENT_LOCAL_DIR, { withFileTypes: true });
...
/**
 * Read agent config entries (definition fields only) from all agent-local/ dirs.
 * Returns [] when agent-local/ is absent or empty.
 */
...
/** Persist a combined agent file to agent-local/<name>/agent.json (with .bak). */
...
/** Delete the agent's entire directory from agent-local/. */
export async function deleteAgentDefinition(agentName: string): Promise<void> {
  const dir = join(AGENT_LOCAL_DIR, agentName);
